...
Below are common examples with the default configurations for links to bugs in popular web-enabled issue tracking systems.

#### Note 
> If your web-based issue tracking system is not listed, or if you have customized the Issue linking URLs on your tracking system, you will need to extract and derive the `url` template by navigating to a sample item, and then substituting tokens such as `%BUGID%` appropriately.
