...
Clicking on pull request icon will allow you to open the pull request in GitHub, and wil also allow you to **Approve** an incoming pull request, without providing further feedback (to leave feedback, use the advanced [commenting review](Integrated-Comments.md) features, in the **Log Window**)
...
When selecting this *merge* node in the **Commits** view, you can see the entire changes which a multi-commit pull request includes and you can [comment](Integrated-Comments.md) on these changes, if necessary. After commenting changes, it's probably a good idea to **Reject** the pull request to signal the initiator of the pull request, that modifications are required before you are willing to pull his changes. If you are fine with a pull request, you may **Merge** it. This will request the GitHub server to merge the pull request and then SmartGit will pull the corresponding branch, so you will have the merged changes locally available.
