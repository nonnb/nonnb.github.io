...
- Users will get their own copy of a source repository (through a process called Cloning) on their local computer, allowing version control even when offline.
...
| Term  | Definition |
| ------------- | ------------- |
| Init  | (`git init`) is the command used to create a new repository locally. You will need to connect to an empty remote repository in order to push changes in your local repository to the remote  |
...
| Checkout | Checkout (`git checkout`) is the Git command used to switch between, or create new branches |
| Staging | Staging is the process whereby you identify the files which have been changed (added, edited or deleted) in your Working Tree, that are to be included in the next commit. Git tracks staged files in the Git [index](The-Index.md). |
...
| Local Repository | The local repository is a (clone of a) repository stored on your local system (in the `.git` folder). You should not directly make changes to the local repository, instead, you should make changes to files in the Working Tree |
...
| Rebase | Rebasing (`git rebase`) provides an alternative to merging two branches together, by instead, creating or appending equivalent commits representing the changes made in the source branch, into the target branch, which removes the appearance of branching from the commit history. In visualization tools, rebased commits appear as a linear sequence of commits on the branch. |
| Squash Commits | Git provides the ability to create a new, single, consolidated commit from a series of smaller commits, essentially the 'rewriting' history in a branch. This can be useful for removing an unnecessarily 'noisy' commit history from your repository, however, this has the downside of losing some of the audit trail of when and whom made changes. |
| Merge (Commit) | A merge commit is a new commit added when two branches are merged. The log will retain information about both branches and visualization tools will be able to show commits made on the divergent branches and where they branched off, and merged together. |
| Fast Forward | Fast forwarding is a technique applied by Git to apply new commits to an existing branch by simply appending each commit to the branch in sequence. e.g. Fast forwarding can be used when `pulling` changes from a remote branch (if no local changes have been made), or when rebasing a branch from new commits held in a side branch. Fast forwarding has an advantage over squash and merge commits in that no new commits are needed. Fast forwarding cannot be done if there are new commits in both source and destination branches. |
| Cherry Pick | Cherry picking (`git cherry-pick`) is an advanced feature used to pull a specific, existing commit, into the current branch. Although cherry-picking can be useful for picking specific features off side branches into a new release, extreme caution is needed to ensure that all dependent commits are cherry-picked in sequential order |
| Cherry Pick | Cherry picking (`git cherry-pick`) is an advanced feature used to pull a specific, existing commit, into the current branch. Although cherry-picking can be useful for picking specific features off side branches into a new release, extreme caution is needed to ensure that all dependent commits are cherry-picked in sequential order |
| [Working Tree](Working-Tree-States.md) | The working tree is the folder on your local system where you can make changes to files in a repository, and then stage, and commit these change |
...
| Secure Shell (SSH) | SSH is an encrypted network protocol, historically used as the means of authenticating and network communication between local and remote repositories. **TODO** See SSH |
| HTTPS | HyperText Transfer Protocol Secure is an alternative protocol for Git to connnect between local and remote repositories.|
| Credential Manager | A Git Credential Manager is a secure tool which assists in retaining a user's credentials as required to access remote repositories. Credential Managers simply HTTPS access to remote repositories, so that users aren't prompted for authentication on each git command.|
| Large File Storage | Git LFS is an extension which stores large binary files (BLOBs) separately from your repositories. This is useful because Git is unable to track 'differences' between binary files in the same way it can with text files such as source code or wiki documents, and adding BLOBs directly to your Git repository can bloat the storage required, and impact performance of commands between local and remote repositories.|
| [Submodules](Submodules.md) | Submodules allow a common repository (e.g. a library project) to be referenced by one or more 'parent' repositories. This allows reuse of common code without maintaining separate copies of the common repository. |
