---
redirect_from:
  - /SmartGit/Latest/Installation-and-Files.html
  - /SmartGit/Latest/Installation-and-Files
---
...
-   `logs/*` contains debug log information, for example, `logs/log.txt.0` contains the most recent logging.
    It can be configured via `logger.properties`. If you delete or move this file, SmartGit will return to its default logging settings.
...
-   `repository-cache` stores all cached information about repository states, e.g. what local branch is checked out, whether there are
...
-   `tools.yml` stores *external* tools which have been configured in [Preferences](../GUI/Preferences/Preferences.md). 
...
e.g. 18.2) and you are giving the current preview version a try ("*newer version*", e.g. 19.1). 
Once you are first installing/starting the *newer version* it will copy over settings from the *older version*. 
From that point of time, settings will diverge. 
...
5.  From the command line or file manager, make a backup of all files from the *new settings directory*, then delete all files to finally have
    an empty directory
...
The `control`-file only *configures* which binaries are part of the build by linking to the actual binaries which are stored in the `repo`-subdirectory 
and which are shared among all builds.

Each new build has a corresponding, digitally-signed control file which contains information about all required application files with their
download location and the expected file content hash. 
...
This uses the `control` file from the *Updates* directory to determine which updated SmartGit files to launch that
